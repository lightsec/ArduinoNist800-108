/*
  LinkedList.h 
  Written: Obed Isai
  Lnk: https://gist.github.com/obedrios/2957439
  Modified: Daniela Ventura
*/
...
template<typename T> void insert_node_first(Node<T> *root, Node<T> *node){
  Node<T>* temp = create_node(node->value);
  temp = root->next;
  root->next = node;
  node->next = temp;
}
...
/* 
template<typename T> void display_list_serial(Node<T> *root){
  Serial.begin(9600);
  while(root->next !=0){
    root = root->next;
